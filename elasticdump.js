...
class elasticdump extends EventEmitter {
  constructor(input, output, timestamp, options) {
    super();
    this.input = input;
    this.output = output;
    ...
    if (
      output !== "$" &&
      (this.options.toLog === null || this.options.toLog === undefined)
    ) {
    ...
    if (this.options.type === "data" && this.options.transform) {
      ...
      this.modifiers = this.options.transform.map(transform => {
        if (transform[0] === "@") {
          ...
          const modificationScriptText =
            "(function(doc) { " + transform + " })";
  ...
  dump(callback, continuing, limit, offset, totalWrites) {
    ...
    if (continuing !== true) {
      ...
      if (self.options.offset) {
        ...
        self.log(
          "  * Using an offset doesn't guarantee that the offset rows have already been written, please refer to the HELP text."
        );
      }
      if (self.modifiers.length) {
        self.log(
          "Will modify documents using these scripts: " + self.options.transform
        );
      }
    }

    this._loop(limit, offset, totalWrites).then(
      totalWrites => {
        if (typeof callback === "function") {
          return callback(null, totalWrites);
        }
      },
      error => {
        if (typeof callback === "function") {
          return callback(error /*, totalWrites */);
        }
      }
    );
  }
  ...
  async _loop(limit, offset, totalWrites) {
    ...
    const ignoreErrors =
      self.options["ignore-errors"] === true ||
      self.options["ignore-errors"] === "true";
    const queue = new PQueue({
      ...
      carryoverConcurrencyCount:
        self.options["carryoverConcurrencyCount"] || false
    });
    ...
    for (;;) {
      ...
      overlappedIoPromise = set(data, limit, offset).then(writes => {
        totalWrites += writes;
        if (data.length > 0) {
          self.log(
            "sent " +
